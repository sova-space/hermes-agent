...
1. **Silence** other agents' `@`-addressed slash commands in multi-bot group
   ...
   reply from Hermes).
2. **Host** the `/project` and `/do` Doer command surface — pick an active
   project, then dispatch tasks to it from a group chat.
...
| `commands.py`        | `/project` + `/do` handlers, and the registry pattern for adding more |
...
`if not chat_id: return` guard ate every reply. The result: `/project` and
`/do` showed up in Telegram's menu and did *visibly nothing* when invoked —
no error, no reply, nothing in the logs (the gateway catches and swallows
hook exceptions, and this wasn't even an exception — just silently wrong data).
...
1. Write `def handle_foo(ctx: CommandContext) -> dict | None`. Reply with
   ...
   let it.
2. Add it to `COMMANDS` in `commands.py` (or extend `route()` for prefix
   families like `/do_<project>`).
