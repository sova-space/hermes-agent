...
async def balance(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(format_balance(accounts), parse_mode=PARSE_MODE)
...
async def sync(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await update.message.reply_text("🔄 Syncing…", parse_mode=PARSE_MODE)
        ...
        await update.message.reply_text(
            format_sync_status(status), parse_mode=PARSE_MODE
        )
    except Exception as e:
