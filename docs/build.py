...
def highlight_code(code: str, env: bool = False) -> str:
    ...
    for line in code.splitlines():
        if env and "=" in line:
            ...
            lines.append(
                f'<span class="kw">{key}</span>=<span class="val">{val}</span>'
            )
        else:
...
def render(d: dict) -> str:
    ...
    # ── Hero badges ──────────────────────────────────────────────────────────
    hero_badges = "".join(f'<span class="badge">{b}</span>' for b in d["hero_badges"])
    ...
    # ── Pocket bars ───────────────────────────────────────────────────────────
    def _pocket_html(p: dict) -> str:
        style_attr = f' style="color:{p["grad_from"]}"' if p.get("overspent") else ""
        return (
            f'<div class="pocket" data-pct="{p["pct"]}" data-color="{p["grad_from"]}" '
            f'data-delay="{p["delay"]}">'
            f'<div class="pocket-pct"{style_attr}>'
            f"{p['pct']}%</div>"
    ...
    pockets_visual = "".join(_pocket_html(p) for p in d["pockets"])
